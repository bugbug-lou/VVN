...
def evonorm(inputs, train, layer=LAYER_EVONORM_B0, nonlinearity=True,
            init_zero=False, data_format='channels_first'):
  ...
  with tf.variable_scope(None, default_name='evonorm'):
    ...
    if nonlinearity:
      ...
      if layer == LAYER_EVONORM_S0:
        ...
        left = _batch_std(inputs, data_format=data_format, training=train)
...
def dropblock(net, train, keep_prob, dropblock_size,
              data_format='channels_first'):
  ...
  if not train or keep_prob is None:
    return net
...
def residual_block(inputs, filters, train, strides,
  ...
  Args:
    ...
    train: `bool` for whether the model is in training.
  ...
  shortcut = inputs
  if pre_activation:
    inputs = norm_activation(inputs, train, data_format=data_format,
  ...
  if use_projection:
    ...
    if not pre_activation:
      shortcut = norm_activation(
          shortcut, train, nonlinearity=False, data_format=data_format,
          layer=norm_act_layer)
  ...
  if pre_activation:
    ...
    inputs = norm_activation(
        inputs, train, nonlinearity=False, init_zero=True,
        data_format=data_format, layer=norm_act_layer)

    return tf.nn.relu(inputs + shortcut)


def bottleneck_block(inputs, filters, train, strides,
                     use_projection=False, data_format='channels_first',
                     dropblock_keep_prob=None, dropblock_size=None,
                     pre_activation=False, norm_act_layer=LAYER_BN_RELU,
                     weight_decay=0.0):
  ...
  if use_projection:
    ...
    if not pre_activation:
      shortcut = norm_activation(
          shortcut, train, nonlinearity=False,
          data_format=data_format, layer=norm_act_layer)
  shortcut = dropblock(
      shortcut, train=train, data_format=data_format,
  ...
  inputs = norm_activation(inputs, train, data_format=data_format,
                           layer=norm_act_layer)
  inputs = dropblock(
      inputs, train=train, data_format=data_format,
      keep_prob=dropblock_keep_prob, dropblock_size=dropblock_size)
  ...
  if pre_activation:
    ...
    inputs = norm_activation(inputs, train, nonlinearity=False,
...
def block_group(inputs, filters, block_fn, blocks, strides, train, name,
  ...
  inputs = block_fn(inputs, filters, train, strides,
  ...
  for _ in range(1, blocks):
    inputs = block_fn(inputs, filters, train, 1,
                      data_format=data_format,
                      dropblock_keep_prob=dropblock_keep_prob,
                      dropblock_size=dropblock_size,
                      pre_activation=pre_activation,
...
def resnet_generator(block_fn, layers, num_classes=None, get_features=False,
  ...
  Returns:
    Model `function` that takes in `inputs` and `train` and returns the
  ...
  def model(inputs, train=True, **kwargs):
    ...
    endpoints['block0'] = tf.identity(inputs, 'block0')
    if not pre_activation:
      inputs = norm_activation(inputs, train, data_format=data_format,
                               layer=norm_act_layer)
    ...
    inputs = custom_block_group(
        ...
        strides=1, train=train, name='block_group1',
        dropblock_keep_prob=dropblock_keep_probs[0])
    endpoints['block1'] = tf.identity(inputs, 'block1')
    inputs = custom_block_group(
        inputs=inputs, filters=128, block_fn=block_fn, blocks=layers[1],
        strides=2, train=train, name='block_group2',
        dropblock_keep_prob=dropblock_keep_probs[1])
    endpoints['block2'] = tf.identity(inputs, 'block2')
    inputs = custom_block_group(
        inputs=inputs, filters=256, block_fn=block_fn, blocks=layers[2],
        strides=2, train=train, name='block_group3',
        dropblock_keep_prob=dropblock_keep_probs[2])
    endpoints['block3'] = tf.identity(inputs, 'block3')
    inputs = custom_block_group(
        inputs=inputs, filters=512, block_fn=block_fn, blocks=layers[3],
        strides=2, train=train, name='block_group4',
        dropblock_keep_prob=dropblock_keep_probs[3])
    endpoints['block4'] = tf.identity(inputs, 'block4')
    ...
    inputs = tf.layers.average_pooling2d(
        ...
        data_format=data_format)
    endpoints['pool'] = tf.identity(inputs, 'final_avg_pool')
    if num_classes is not None:
      ...
      endpoints['logits'] = tf.identity(inputs, 'final_dense')
  ...
  model.default_image_size = 256
  return model

def resnet(resnet_depth, num_classes=None, data_format='channels_last',
           dropblock_keep_probs=None, dropblock_size=None, get_features=False,
           pre_activation=True, norm_act_layer=LAYER_BN_RELU,
           weight_decay=0.0):
  ...
  return resnet_generator(
      params['block'], params['layers'], num_classes, get_features=get_features,
