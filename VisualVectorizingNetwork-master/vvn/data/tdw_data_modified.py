...
def auto_fill_dataset_path(dataset, dataprefix=None, validate_on_train_data=False):
    ...
    dataprefix = dataprefix if dataprefix is not None else 'C:/shapestacks/frc_35'
...
class TdwSequenceDataProvider(DataProvider):
    ...
    def get_data_params(batch_size,
                        ...
                        dataprefix='C:/',
                        sources=[],
                        n_tr_per_dataset=(32*2389),
                        n_val_per_dataset=(32*514),
    ...
    def build_one_dataset(self, curr_data):
        ...
        # the same
        file_datasets = {source: tf.data.Dataset.list_files(curr_files, shuffle=False) for source, curr_files in source_lists.items()}
        ...
        def _fetch_dataset(filename):
	        ...
	        dataset = (filename, buffer_size=buffer_size)
	        return dataset
...
if __name__ == '__main__':
    ...
    for i in range(NUM_STEPS):
        inputs = data_provider.input_fn(BATCH_SIZE)
    ...
    with open('C:/shapestacks/new_tdw_tfrdata.pkl', 'wb') as f:
