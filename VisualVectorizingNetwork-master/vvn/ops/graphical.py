...
def euclidean_dist2(v1, v2, thresh=None, **kwargs):
    ...
    # thresh = tf.Print(thresh, [N*F, tf.reduce_mean(tf.reduce_sum(tf.cast(dists2 < thresh, tf.float32), axis=[1,2]))], message='thresh_edges')
    adjacency = tf.cast(dists2 < thresh, tf.bool)

    return adjacency

def compute_adjacency_from_features(features, k=1, metric=euclidean_dist2, metric_kwargs={}, symmetric=False):
    '''
    Inputs
    ...
    if metric_kwargs.get('add_coordinates', False):
        ...
        features = tf.concat([features, hc, wc], axis=3)
        C += 2
    ...
    if symmetric:
        ...
        dh = tf.floordiv(k_inds, ksize) - k # in [-k,k]
        dw = tf.floormod(k_inds, ksize) - k # in [-k,k]
...
def graphconv_pairwise(nodes,
    ...
    :param adjacency: Tensor(batch_size, time, num_nodes, num_nodes, edge_dim),
        adjacency
    :param node_dims: list([int, int, ...], node feature dimensions for hidden
    ...
    :return nodes: Tensor(batch_size, time, num_nodes, node_dims[-1]), convolved nodes
    '''
    ...
    if scale_diffs_by_effects:
        assert node_dims[-1] == nodes.shape.as_list()[-1], \
    ...
    activations = [activation] * len(node_dims)
    ...
    effects = mlp(pairs, node_dims, activations,
...
def graphconv_pairwise_from_inds(
    ...
    effects = mlp(pairs, node_dims, activations, scope=scope, share_weights=share_weights, **kwargs) # [num_edges, node_dims[-1]]
...
def find_nearest_k_node_inds(nodes, kNN=5, nn_dims=[-4,-2], dmax=10000., **kwargs):
    '''
    ...
    if kNN < N:
        ...
        nearest_k_inds = tf.reshape(tf.range(N, dtype=tf.int32), [1,1,N]) * tf.ones([B,N,N], dtype=tf.int32)
