...
def compute_segments_by_label_prop(edges, size, num_steps=10, defensive=False, synchronous=False, mode='index'):
    '''
    ...
    if synchronous:
        print("synchronous image LP")
        if len(shape) == 3:
            B,N,K = edges.shape.as_list()
            newshape = tf.concat([tf.cast([B], tf.int32), size, tf.cast([K], tf.int32)], axis=0)
            segment_ids, num_segments = labelprop_image_sync(tf.reshape(edges, newshape), num_steps, mode=mode)
        elif len(shape) == 4:
            segment_ids, num_segments = labelprop_image_sync(edges, num_steps, mode=mode)
    else:
        print("Asynchronous image LP")
        if len(shape) == 4:
...
def labelprop_fc_sync(valid_nodes, edges, num_steps=10, noise=0.001, seed=0, tau=0.0, labels_init=None):
    '''
    ...
    if tau > 0.:
        print("kmat", kmat.shape.as_list())
        import pdb
        pdb.set_trace()

    # init labels as one-hot identity matrix [B,N,N]
    print("labels init in lp", N, labels_init)
    ...
    rn = noise * tf.random_normal(shape=[B,N], seed=seed)[:,tf.newaxis] # [B,1,N] noise same in each row
...
def labelprop_image_sync(edges, num_steps=10, mode='index', noise=0.001, seed=0, labels_init=None):
    ...
    r = (ksize - tf.constant(1, tf.int32)) / tf.constant(2, tf.int32)
